...
public abstract class TrendPostOpinionExpandableRecyclerAdapter<TPPVH extends TrendPostParentViewHolder, TPCVH extends TrendPostChildViewHolder>
        extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements TrendPostParentViewHolder.ParentListItemExpandCollapseListener
{
    ...
    private ArrayList<? extends ParentArrayListItem> mParentItemList;
    private ExpandCollapseListener mExpandCollapseListener;
    private ArrayList<RecyclerView> mAttachedRecyclerViewPool;
    ...
    TrendPostOpinionExpandableRecyclerAdapter(ArrayList<? extends ParentArrayListItem> parentItemList)
    {
        super();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType)
    {
        if (viewType == TYPE_PARENT) {
            TPPVH pvh = onCreateTrendPostParentViewHolder(viewGroup);
            pvh.setParentListItemExpandCollapseListener(this);
            return pvh;
        } else if (viewType == TYPE_CHILD) {
            return onCreateTrendPostChildViewHolder(viewGroup);
        } else {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (listItem instanceof ParentWrapper) {
            TPPVH parentViewHolder = (TPPVH) holder;
            ...
            onBindTrendPostParentViewHolder(parentViewHolder, position, parentWrapper.getParentListItem());
        } else if (listItem == null) {
            throw new IllegalStateException("Incorrect ViewHolder found");
        } else {
            onBindTrendPostChildViewHolder((TPCVH) holder, position, listItem);
        }
    }

    public abstract TPPVH onCreateTrendPostParentViewHolder(ViewGroup parentViewGroup);

    public abstract TPCVH onCreateTrendPostChildViewHolder(ViewGroup childViewGroup);

    public abstract void onBindTrendPostParentViewHolder(TPPVH parentViewHolder, int position, ParentArrayListItem parentListItem);

    public abstract void onBindTrendPostChildViewHolder(TPCVH childViewHolder, int position, Object childListItem);
    ...
    public ArrayList<? extends ParentArrayListItem> getParentItemList()
    {
    ...
    private void expandParent(int parentIndex) {
    ...
    private void expandParent(ParentArrayListItem parentListItem) {
    ...
    public void expandAllParents() {
        for (ParentArrayListItem parentListItem : mParentItemList) {
            expandParent(parentListItem);
        }
    }

    private void collapseParent(int parentIndex) {
        int parentWrapperIndex = getParentWrapperIndex(parentIndex);
    ...
    private void collapseParent(ParentArrayListItem parentListItem) {
        ParentWrapper parentWrapper = getParentWrapper(parentListItem);
        int parentWrapperIndex = mItemList.indexOf(parentWrapper);
        if (parentWrapperIndex == -1) {
            return;
    ...
    private void expandViews(ParentWrapper parentWrapper, int parentIndex) {
        ...
        for (RecyclerView recyclerView : mAttachedRecyclerViewPool) {
            viewHolder = (TPPVH) recyclerView.findViewHolderForAdapterPosition(parentIndex);
    ...
    private void expandParentListItem(ParentWrapper parentWrapper, int parentIndex, boolean expansionTriggeredByListItemClick)
    {
        if (!parentWrapper.isExpanded())
        {
            ...
            ArrayList<?> childItemList = parentWrapper.getChildItemList();
    ...
    public void notifyParentItemInserted(int parentPosition) {
        ParentArrayListItem parentListItem = mParentItemList.get(parentPosition);
    ...
    public void notifyParentItemRangeInserted(int parentPositionStart, int itemCount) {
        ...
        for (int i = parentPositionStart; i < parentPositionEnd; i++) {
            ParentArrayListItem parentListItem = mParentItemList.get(i);
    ...
    private int addParentWrapper(int wrapperIndex, ParentArrayListItem parentListItem) {
        int sizeChanged = 1;
    ...
    private int changeParentWrapper(int wrapperIndex, ParentArrayListItem parentListItem) {
        ParentWrapper parentWrapper = (ParentWrapper) mItemList.get(wrapperIndex);
        parentWrapper.setParentListItem(parentListItem);
        int sizeChanged = 1;
        if (parentWrapper.isExpanded()) {
            ArrayList<?> childItems = parentWrapper.getChildItemList();
    ...
    public void notifyParentItemMoved(int fromParentPosition, int toParentPosition)
    {
        ...
        else
        {
          int sizeChanged = 0;
            ...
            mItemList.add(toWrapperIndex + childOffset, fromParentWrapper);
            ArrayList<?> childItemList = fromParentWrapper.getChildItemList();
    ...
    public void notifyChildItemRangeInserted(int parentPosition, int childPositionStart, int itemCount) {
        ...
        if (parentWrapper.isExpanded()) {
            ParentArrayListItem parentListItem = mParentItemList.get(parentPosition);
            ArrayList<?> childList = parentListItem.getChildItemList();
            Object child;
            for (int i = 0; i < itemCount; i++) {
    ...
    private ParentWrapper getParentWrapper(ParentArrayListItem parentListItem) {
        int listItemCount = mItemList.size();
        for (int i = 0; i < listItemCount; i++) {

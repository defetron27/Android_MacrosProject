...
public class SingleVideoPlayerManager implements VideoPlayerManager<MetaData>, VideoPlayerManagerCallback, MediaPlayerWrapper.MainThreadMediaPlayerListener {
    ...
    @Override
    public void playNewVideo(MetaData currentItemMetaData, VideoPlayerView videoPlayerView, File videoFile, Context context)
    {
        if(SHOW_LOGS) Logger.v(TAG, ">> playNewVideo, videoPlayer " + videoPlayerView + ", mCurrentPlayer " + mCurrentPlayer + ", videoPlayerView " + videoPlayerView);
        ...
        boolean isAlreadyPlayingTheFile =
                mCurrentPlayer != null &&
                        videoFile.equals(mCurrentPlayer.getVideoUrlDataSource());
        ...
        if(currentPlayerIsActive){
            if(isInPlaybackState() && isAlreadyPlayingTheFile){
                ...
                /** 4. */
                startNewPlayback(currentItemMetaData, videoPlayerView, videoFile,context);
            }
        } else {
            /** 4. */
            startNewPlayback(currentItemMetaData, videoPlayerView, videoFile,context);
        ...
        if(SHOW_LOGS) Logger.v(TAG, "<< playNewVideo, videoPlayer " + videoPlayerView + ", videoUrl " + videoFile);
    }
    ...
    private void startNewPlayback(MetaData currentItemMetaData, VideoPlayerView videoPlayerView, File  storageFile, Context context)
    {
        ...
        startPlayback(videoPlayerView, storageFile, context);
    }
    ...
    private void startPlayback(VideoPlayerView videoPlayerView, File videoFile, Context context)
    {
        if(SHOW_LOGS) Logger.v(TAG, "startPlayback");

        mPlayerHandler.addMessages(Arrays.asList(
                new CreateNewPlayerInstance(videoPlayerView, this),
                new SetFileDataSourceMessage(videoPlayerView, videoFile, this,context),
                new Prepare(videoPlayerView, this),
                new Start(videoPlayerView, this)
        ));
    }

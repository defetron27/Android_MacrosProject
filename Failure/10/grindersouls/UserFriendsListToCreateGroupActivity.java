        ...
        createGroupUsingSelectedMembersButton.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                if (Incoming.equals("ViewUserGroupProfileActivity"))
                {
                    ...
                    for (int i = 0; i < selectedFriendsToSend.size(); i++)
                    {
                        ...
                        FriendsListRef.child(selectedFriendsToSend.get(i)).addValueEventListener(new ValueEventListener()
                        {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot)
                            {
                                ...
                                NewAddedMembers.put(name, selectedFriendsToSend.get(finalI));
                            }
                    ...
                    UserGroupsRef.child(AdminKey).child(GroupPosition).child("no_of_new_group_members").removeValue().addOnCompleteListener(new OnCompleteListener<Void>()
                    {
                        @Override
                        public void onComplete(@NonNull Task<Void> task)
                        {
                            UserGroupsRef.child(AdminKey).child(GroupPosition).child("no_of_new_group_members").updateChildren(NewAddedMembers).addOnCompleteListener(new OnCompleteListener<Void>()
                            {
                                @Override
                                public void onComplete(@NonNull Task<Void> task)
                                {
                                    Intent viewGroupProfileIntent = new Intent(UserFriendsListToCreateGroupActivity.this, ViewUserGroupProfileActivity.class);
                                    viewGroupProfileIntent.putExtra("admin",AdminKey);
                                    viewGroupProfileIntent.putExtra("group_position",GroupPosition);
                                    viewGroupProfileIntent.putExtra("group_name",GroupName);
                                    viewGroupProfileIntent.putExtra("date",CreatedGroupDate);
